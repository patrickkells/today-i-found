...
import { validateCopyStyle } from "./copy-style.js";
  ...
  for (const candidate of ledger.candidates) {
    ...
    if (!decision || decision.status === "pending") errors.push(`${candidate.discoveryId} still has a pending decision.`);
      ...
      if (!copy?.promptVersion?.trim()
        ...
        if (copy.promptVersion !== policy.copy.promptVersion) {
          errors.push(`${candidate.discoveryId} copy.promptVersion must be ${policy.copy.promptVersion}.`);
        }
        for (const error of validateCopyStyle(copy, policy.copy)) {
          errors.push(`${candidate.discoveryId} ${error}`);
        }
      }
