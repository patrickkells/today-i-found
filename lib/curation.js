...
import { PRIMARY_CATEGORIES, STRUCTURED_TAG_PATTERN, freshnessLimitFor } from "../shared/editorial-contract.js";
...
const BROAD_CATEGORIES = new Set(PRIMARY_CATEGORIES);
...
export function validateEdition(edition, { allowLegacy = false, candidatePool = false } = {}) {
  ...
  if (edition?.schemaVersion === 2) return validateSchemaTwo(edition);
  return validateSchemaThree(edition, { candidatePool });
}

function validateSchemaTwo(edition) {
  const errors = [];
  ...
  if (edition?.schemaVersion !== 2) {
    errors.push("edition.schemaVersion must be 2.");
  }
  if (!Array.isArray(items) || items.length < 1 || items.length > 15) {
    errors.push("Edition must contain between 1 and 15 items.");
  }
  ...
  for (const [index, item] of (Array.isArray(items) ? items : []).entries()) {
    ...
    validateFreshnessEvidence(edition, item, label, errors);
...
function validateSchemaThree(edition, { candidatePool = false } = {}) {
  const errors = [];
  const items = edition?.items;
  if (!isNonEmptyString(edition?.date)) errors.push("edition.date is required.");
  else if (!isValidCalendarDate(edition.date)) errors.push("edition.date must be a valid calendar date.");
  ...
  const minimum = candidatePool ? 0 : policy.edition.minItems;
  const maximum = candidatePool ? policy.edition.maxCandidateItems : policy.edition.maxItems;
  if (!Array.isArray(items) || items.length < minimum || items.length > maximum) {
    errors.push(`Edition must contain between ${minimum} and ${maximum} items.`);
  }
  if (!candidatePool) validateDiscoveryStats(edition?.discoveryStats, items, errors);
  ...
  for (const [index, item] of (Array.isArray(items) ? items : []).entries()) {
    ...
    if (!isNonEmptyString(item?.id)) errors.push(`${label} item id must be a non-empty string.`);
    else if (itemIds.has(item.id)) errors.push(`${label} must have a unique item id.`);
    else itemIds.add(item.id);

    if (!BROAD_CATEGORIES.has(item?.category)) errors.push(`${label} has an invalid category.`);
    if (!EDITORIAL_TIERS.has(item?.editorialTier)) errors.push(`${label} has an invalid editorialTier.`);
    if (!isNonEmptyString(item?.rankingRationale)) errors.push(`${label} is missing rankingRationale.`);
    if (!Array.isArray(item?.tags) || item.tags.length < 1 || item.tags.length > 12
      ...
      errors.push(`${label} must have one to twelve structured tags.`);
    }
    if (item?.publicationDate && !isValidCalendarDate(item.publicationDate)) {
      errors.push(`${label} publicationDate must be a valid calendar date.`);
    }
    for (const field of REQUIRED_SIGNAL_FIELDS) {
      if (!isNonEmptyString(item?.signal?.[field])) errors.push(`${label} is missing signal.${field}.`);
    }
    if (hasOwnProperty(item, "timeToTry")) errors.push(`${label} must not include retired timeToTry.`);
    if (hasOwnProperty(item, "caveat")) errors.push(`${label} must not include caveat.`);
    if (hasOwnProperty(item, "experiment")) errors.push(`${label} must not include experiment.`);
    validateSchemaThreeSource(edition, item, label, errors);
    if (isProhibitedTopic(item ?? {}).prohibited) errors.push(`${label} covers a prohibited topic.`);
  }
  return errors;
}

function validateDiscoveryStats(stats, items, errors) {
  const fields = ["rawCandidates", "clusteredCandidates", "eligibleCandidates", "publishedItems", "trendingReviewed", "explorationItems"];
  for (const field of fields) {
    if (!Number.isInteger(stats?.[field]) || stats[field] < 0) errors.push(`edition.discoveryStats.${field} must be a non-negative integer.`);
  }
  if (fields.some((field) => !Number.isInteger(stats?.[field]) || stats[field] < 0)) return;
  const itemCount = Array.isArray(items) ? items.length : 0;
  if (stats.publishedItems !== itemCount) errors.push("edition.discoveryStats.publishedItems must equal edition.items.length.");
  if (stats.clusteredCandidates > stats.rawCandidates) errors.push("edition.discoveryStats.clusteredCandidates cannot exceed rawCandidates.");
  if (stats.eligibleCandidates > stats.clusteredCandidates) errors.push("edition.discoveryStats.eligibleCandidates cannot exceed clusteredCandidates.");
  if (stats.publishedItems > stats.eligibleCandidates) errors.push("edition.discoveryStats.publishedItems cannot exceed eligibleCandidates.");
  if (stats.trendingReviewed > stats.rawCandidates) errors.push("edition.discoveryStats.trendingReviewed cannot exceed rawCandidates.");
  if (stats.explorationItems > Math.floor(itemCount * policy.feedback.explorationRatio)) {
    errors.push("edition.discoveryStats.explorationItems exceeds the exploration ceiling.");
  }
}

function validateSchemaThreeSource(edition, item, label, errors) {
  const source = item?.source;
  if (!source?.url || !["primary", "original-reporting"].includes(source.kind) || source.verification?.status !== "verified") {
    errors.push(`${label} needs a verified primary or original-reporting source.`);
    return;
  }
  if (!isNonEmptyString(source.publisher)) errors.push(`${label} is missing source.publisher.`);
  if (!isNonEmptyString(source.title)) errors.push(`${label} is missing source.title.`);
  if (!isNonEmptyString(source.verification?.method)) errors.push(`${label} is missing source.verification.method.`);
  if (!isValidCalendarDate(source.verification?.verifiedAt)) errors.push(`${label} source.verification.verifiedAt must be a valid calendar date.`);
  if (policy.freshness.requireVerificationOnEditionDate && source.verification?.verifiedAt !== edition?.date) {
    errors.push(`${label} source verification must be performed on the edition date.`);
  }
  try { canonicalizeUrl(source.url); } catch { errors.push(`${label} has an invalid source URL.`); }

  const evidence = source.evidence;
  if (!policy.freshness.directEvidenceTypes.includes(evidence?.type)) errors.push(`${label} has an invalid source.evidence.type.`);
  if (!isNonEmptyString(evidence?.dateBasis)) errors.push(`${label} is missing source.evidence.dateBasis.`);
  if (!policy.freshness.productStatuses.includes(evidence?.productStatus)) errors.push(`${label} has an invalid source.evidence.productStatus.`);

  if (policy.freshness.currentDiscoveryTypes.includes(evidence?.type)) {
    if (evidence?.freshnessClass !== "current-discovery") errors.push(`${label} Trending evidence must use current-discovery freshness.`);
    if (evidence?.observedAt !== edition?.date) errors.push(`${label} Trending evidence must be observed on the edition date.`);
    if (!Array.isArray(evidence?.trendingWindows) || !evidence.trendingWindows.length
      || evidence.trendingWindows.some((window) => !policy.discovery.githubTrendingWindows.includes(window))) {
      errors.push(`${label} Trending evidence must name a configured daily or weekly window.`);
    }
    return;
  }

  const limit = freshnessLimitFor(item, policy);
  if (!Number.isInteger(limit)) errors.push(`${label} has an invalid source.evidence.freshnessClass.`);
  if (isValidCalendarDate(edition?.date) && isValidCalendarDate(item?.publicationDate) && Number.isInteger(limit)) {
    const ageDays = (calendarDay(edition.date) - calendarDay(item.publicationDate)) / 86_400_000;
    if (ageDays < 0) errors.push(`${label} publicationDate is after the edition date.`);
    if (ageDays > limit) errors.push(`${label} event must be published within the previous ${limit === 7 ? "seven" : "fourteen"} days.`);
  }
  if (source.kind === "original-reporting" && evidence?.type !== "original-reporting") {
    errors.push(`${label} original reporting must use original-reporting evidence.`);
  }
  if (evidence?.productStatus === "migration") {
    const copy = `${item?.title ?? ""} ${item?.summary ?? ""}`;
    if (evidence.type !== "migration-notice" || !/\b(deprecat|retir|shut(?:ting)? down|migrat|sunset)/i.test(copy)) {
      errors.push(`${label} must present a deprecated product as an explicitly labeled migration notice.`);
    }
  }
}

function validateFreshnessEvidence(edition, item, label, errors) {
  const rules = policy.freshness;
  if (!rules || !isValidCalendarDate(edition?.date) || edition.date < rules.enforceForEditionsOnOrAfter) return;

  if (isValidCalendarDate(item?.publicationDate)) {
    const ageDays = (calendarDay(edition.date) - calendarDay(item.publicationDate)) / 86_400_000;
    if (ageDays < 0) errors.push(`${label} publicationDate is after the edition date.`);
    if (ageDays > rules.maxAgeDays) errors.push(`${label} event must be published within the previous seven days.`);
  }

  const evidence = item?.source?.evidence;
  if (!rules.directEvidenceTypes.includes(evidence?.type)) {
    errors.push(`${label} needs direct dated event evidence from a release, changelog, announcement, paper, repository release, dated update, or migration notice.`);
  }
  if (!isNonEmptyString(evidence?.dateBasis)) errors.push(`${label} is missing source.evidence.dateBasis.`);
  if (!rules.productStatuses.includes(evidence?.productStatus)) errors.push(`${label} has an invalid source.evidence.productStatus.`);
  if (rules.requireVerificationOnEditionDate && item?.source?.verification?.verifiedAt !== edition.date) {
    errors.push(`${label} source verification must be performed on the edition date.`);
  }

  if (evidence?.productStatus === "migration") {
    const copy = `${item?.title ?? ""} ${item?.summary ?? ""}`;
    const isLabeled = evidence.type === "migration-notice" && /\b(deprecat|retir|shut(?:ting)? down|migrat|sunset)/i.test(copy);
    if (!isLabeled) errors.push(`${label} must present a deprecated product as an explicitly labeled migration notice.`);
  }
}
