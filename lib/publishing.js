...
async function registerEditionThroughProxy(edition, registrationProxyUrl, fetchImpl) {
  const url = new URL(registrationProxyUrl);
  if (url.protocol !== "http:" || url.hostname !== "127.0.0.1") throw new Error("Registration proxy must use local loopback HTTP.");
...
export async function publishEdition({
  ...
  if (registerUrl && registrationProxyUrl) throw new Error("Use only one registration boundary.");
  if (registrationProxyUrl) await registerEditionThroughProxy(edition, registrationProxyUrl, fetchImpl);
  else if (registerUrl) await registerEdition(edition, registerUrl, curatorToken, fetchImpl);
